Shader "HachigayoLab/PolychoralAccessoryUniform" {
  Properties {
    ...
    _Root1 ("Root1", Vector) = (1, 0, 0, 0)
    _Root2 ("Root2", Vector) = (0, 1, 0, 0) 
    _Root3 ("Root3", Vector) = (0, 0, 1, 0)
    _Root4 ("Root4", Vector) = (0, 0, 0, 1)
  }
  SubShader {
    ...
    Pass
    {
      ...
      struct appdata {
        ...
        float4 matrix1 : TEXCOORD4;
        float4 matrix2 : TEXCOORD5;
        float4 matrix3 : TEXCOORD6;
        float4 matrix4 : TEXCOORD7;
      };
      ...
      v2f vert (appdata v) {
        // --- apply matrix ---
        float4x4 mat = float4x4(v.matrix1, v.matrix2, v.matrix3, v.matrix4);
        float4 params = float4(abs(v.vertex.x), abs(v.vertex.y), abs(v.vertex.z), 0) * 3;
        params /= clamp(params.x + params.y + params.z, 1, 1e9);
        params.w = 1 - params.x - params.y - params.z;
        float4 coord = mul(_Root1 * params.x + _Root2 * params.y + _Root3 * params.z + _Root4 * params.w, mat);
        // --- calculate rotation ---
        ...
        float4 pos4d = RotV(Rotate(Rotate(mul(coord, rot), _AutoRot1From, to1), _AutoRot2From, to2), r);
        // --- projection ---
        ...
        if (/*(_Glass < 0.5) &&*/ ((pos4d.w * _Distance - dot(v.normal, coord) - 1e-9) /** (-_Inversed)*/ < 0)) {
          pos = float4(0, 0, 0, 0);
        }
        v2f o;
