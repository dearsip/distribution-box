Shader "HachigayoLab/4DSDFSliceStacker/4S3_Grabbable" {
  Properties {
  ...
  SubShader {
    ...
    Pass {
      ...
      v2f vert (appdata v) {
        ...
        float t = length(dir) * 4;
        float4 dr = float4(0, 0, 0, 1);
        if (t > 1e-9) dr = float4(sin(t) * normalize(dir), cos(t));

        dir = float3(_SliceRotXW, _SliceRotYW, _SliceRotZW);
        t = length(dir) * PI;
        ...
        o.slice1 = Rotate( Rotate( Rotate( Rotate( Rotate( float4(0, 1, 0, 0), float4(0, 0, 0, 1), dr), float4(1, 0, 0, 0), float4(cos(_SliceRotXY*PI), sin(_SliceRotXY*PI), 0, 0)), float4(0, 1, 0, 0), float4(0, cos(_SliceRotYZ*PI), sin(_SliceRotYZ*PI), 0)), float4(0, 0, 1, 0), float4(sin(_SliceRotZX*PI), 0, cos(_SliceRotZX*PI), 0)), float4(0, 0, 0, 1), br);
        o.slice2 = Rotate( Rotate( Rotate( Rotate( Rotate( float4(0, 0, 1, 0), float4(0, 0, 0, 1), dr), float4(1, 0, 0, 0), float4(cos(_SliceRotXY*PI), sin(_SliceRotXY*PI), 0, 0)), float4(0, 1, 0, 0), float4(0, cos(_SliceRotYZ*PI), sin(_SliceRotYZ*PI), 0)), float4(0, 0, 1, 0), float4(sin(_SliceRotZX*PI), 0, cos(_SliceRotZX*PI), 0)), float4(0, 0, 0, 1), br);
        o.slice3 = Rotate( Rotate( Rotate( Rotate( Rotate( float4(0, 0, 0, 1), float4(0, 0, 0, 1), dr), float4(1, 0, 0, 0), float4(cos(_SliceRotXY*PI), sin(_SliceRotXY*PI), 0, 0)), float4(0, 1, 0, 0), float4(0, cos(_SliceRotYZ*PI), sin(_SliceRotYZ*PI), 0)), float4(0, 0, 1, 0), float4(sin(_SliceRotZX*PI), 0, cos(_SliceRotZX*PI), 0)), float4(0, 0, 0, 1), br);
        ...
        o.vertex = UnityObjectToClipPos(float4(v.position * _Frame, 1));
      ...
      fixed4 frag (v2f input) : SV_Target {
        ...
        float4 dpos = float4(input.localPos.xyz / Scale, 0);
        Scale = 1 / (Scale * _Frame);
